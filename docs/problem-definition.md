...
1. **🔍 Tự động phát hiện (Privacy-First):** Nhận diện gói đăng ký mới qua bộ lọc chuyển tiếp email chọn lọc và qua Chatbot gia đình (**Telegram Bot**) [5].
...
- **Người đăng ký (Subscriber):** Thành viên trong nhà tạo tài khoản (Xác định bằng Telegram Chat ID).
...
- **Interactive Chatbot (Telegram):** Thành viên forward tin nhắn biến động số dư ngân hàng hoặc chụp ảnh màn hình biên lai đăng ký gửi vào Telegram Bot chung của gia đình [9].
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 📊 **React SPA Dashboard (Telegram Mini App)** | Giao diện quản lý chính của gia đình, hiển thị trực quan danh sách subscription, ngày gia hạn, số tiền và tổng chi phí hàng tháng [10].                                                                                                                                                                                        |
...
| 🔔 **Cooperative Tiered Alerts**               | • **Soft Alert (T-3 days):** Bot gửi tin nhắn riêng qua Telegram nhắc nhở thành viên đã đăng ký gói dịch vụ [10].<br>• **Red Alert (T-24 hours):** Nếu thành viên chưa bấm xác nhận "Keep", Bot sẽ gửi cảnh báo khẩn cấp vào nhóm chat gia đình (Telegram Group) và tag Chủ Thẻ để chuẩn bị phương án khóa thẻ ngân hàng [10]. |
...
- **Backend (Cloudflare Workers):** Xử lý webhook từ Telegram Bot API Webhook, kết hợp Cloudflare Email Routing. Miễn phí 100,000 requests/ngày.
...
- **Telegram Bot** tiếp nhận ảnh chụp biên lai và tin nhắn chuyển tiếp [11].
- Triển khai giao diện quản trị **React SPA** (Cloudflare Pages) nhúng trong Telegram Mini App.
...
- Gửi thông báo đa tầng T-3 (Soft Alert đến người đăng ký qua Telegram private chat) và T-24h (Red Alert đến chủ thẻ qua nhóm chat chung) [11].
