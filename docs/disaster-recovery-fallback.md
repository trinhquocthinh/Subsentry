...
- Thành viên gửi biên lai vào Zalo/Messenger Bot nhưng không nhận được phản hồi, hoặc nhận được tin nhắn báo lỗi hệ thống bóc tách.
...
Zalo OA và Facebook Messenger yêu cầu phản hồi HTTP `200 OK` cực kỳ nhanh (thường trong vòng 2 - 3 giây) sau khi gửi webhook payload. Nếu Cloudflare Worker tốn quá nhiều thời gian để gọi API OpenAI bóc tách hình ảnh (thao tác này có thể mất từ 3 - 5 giây), Zalo/Messenger Server sẽ coi là kết nối bị lỗi và tự động gửi lại (retry) nhiều lần, gây ra hiện tượng trùng lặp yêu cầu và lãng phí chi phí gọi API OpenAI.
...
1.  **Nhận & Xác thực**: Worker tiếp nhận Webhook POST từ Zalo/Messenger.
2.  **Trả về HTTP 200 ngay lập tức**: Phản hồi ngay lập tức cho Zalo/Facebook Server mã trạng thái `200 OK` để xác nhận đã nhận dữ liệu thành công.
