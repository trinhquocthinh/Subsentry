...
Hệ thống được thiết kế tối giản để chạy trên hạ tầng **Serverless (Cloudflare Workers + D1 Database + Telegram Bot)** với chi phí vận hành tiệm cận $0 [2].
...
- **Cơ chế thực thi:**
  ...
  - **SMS/Biến động số dư:** Thành viên chủ động sao chép tin nhắn hoặc chụp ảnh màn hình biên lai gửi vào **Telegram Bot** chung của gia đình [10].
...
| ------------- | ----------------------------- | ----------------------------------------- | ----------------------------------- | ------------------------------------------ |
| **T-3 ngày**  | **Soft Alert** (Nhắc nhở nhẹ) | **Subscriber** (Người đăng ký)            | Telegram Chat (Private)             | Xác nhận: Keep (Giữ) hoặc Kill (Hủy) [10]  |
| **T-24 giờ**  | **Red Alert** (Báo động đỏ)   | **Subscriber** + **Card Owner** (Chủ thẻ) | Nhóm Chat Gia Đình (Telegram Group) | Xác nhận khẩn cấp / Khóa thẻ tạm thời [10] |
...
- Mỗi thực thể Subscription được tạo ra bắt buộc phải liên kết với:
  1. **Subscriber (Telegram Chat ID):** Thành viên trực tiếp sử dụng dịch vụ.
...
Khi Cloudflare Worker nhận dữ liệu thô (Email forwarded hoặc ảnh chụp màn hình gửi qua Telegram Bot), dữ liệu chuyển qua GPT-4o-mini API phải được bóc tách thành định dạng JSON chuẩn với các quy tắc sau:
