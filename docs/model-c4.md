...
graph TB
    ...
    SUB -->|5. Chụp hóa đơn / gửi SMS| TG
    TG -->|6. Đẩy tin nhắn Webhook| SYS

    SYS -->|7. Gửi cảnh báo đa tầng| TG
    TG -->|8. Nhắc nhở riêng tư| SUB
    TG -->|9. Báo động khẩn cấp| OWNER
```
...
graph TB
    ...
    TG_API -->|11. Nhắc nhở / Báo động nhóm| SUB
    TG_API -->|11. Nhắc nhở / Báo động nhóm| OWNER
```
...
##### 3.1 Frontend: Telegram Mini App (React SPA)
...
- **Mô tả nhiệm vụ:**
  ...
  - Tích hợp sâu thông qua Telegram Web App SDK (`window.Telegram.WebApp`) để lấy thông tin `User ID` tự động mà không bắt người dùng đăng nhập lại.
...
- **Mô tả nhiệm vụ:**
  ...
  - Cung cấp các cổng API (Endpoints) tiếp nhận Webhook từ Telegram Bot và hệ thống Mail.
